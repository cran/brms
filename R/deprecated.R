...
brmpars <- function(formula, data = NULL, family = "gaussian", 
  ...
  if (is_ordinal && threshold == "flexible") 
    out <- c(out, "b_Intercept")
  if (is_ordinal && threshold == "equidistant") 
    out <- c(out, "b_Intercept1", "delta")
  if (length(f) && family != "categorical") 
    out <- c(out, "b")
  if (is_ordinal && length(p) || family == "categorical") 
    out <- c(out, "bp")
  if (is_linear && !is(ee$se,"formula") && length(ee$response) == 1) 
    out <- c(out, "sigma")
