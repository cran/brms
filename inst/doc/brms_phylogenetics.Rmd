...
With the exception of `cov_ranef = list(phylo = A)` this is a basic multilevel
model with a varying intercept over species (`phylo` is an indicator of species
in this data set). However, by using the `cov_ranef` argument, we make sure that
species are correlated as specified by the covariance matrix `A`. Setting priors
is not required for achieving good convergence for this model, but it improves
sampling speed a bit. After fitting, the results can be investigated in detail.
...
Let's say we have Fisher's z-transformated correlation coefficients $Zr$ per
...
justified and it is recommended to use a distribution explicitely suited for
