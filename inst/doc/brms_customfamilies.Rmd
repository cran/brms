...
Actually, for this particular example, we could more elegantly apply the addition argument `trials()` as in the basic binomial model. However, since the present vignette is ment to give a general overview of the topic, we will go with the more general method of specifying custom data via `stanvar`. 
...
Since smaller `LOOIC` values indicate better fit, we see that the beta-binomial model fits somewhat better, although the corresponding standard error reveals that the difference is not that substantial.
...
Family functions built natively into **brms** are saver to use and more convenient, as they require much less user input. Unfortunately, the current internal structure of **brms** makes it rather hard for an outside developer to implement a native family herself. If you feel that your custom family is general enough to be useful to other users, please feel free to open an issue on [GitHub](https://github.com/paul-buerkner/brms/issues), in which you provide general information about your family as well as all the necessary functions described above. Provided that you convince me, I will then go ahead and make the required changes to the **brms** code base.
