...
which require additional input by the user. These are `pp_expect`,
...
`log_lik` function, except that we are now creating random samples of the
response instead of log-liklihood values. Again, we are using an exposed
...
When defining the `pp_expect` function, you have to keep in mind that it has only a
...
$\text{E}(y) = \mu T$ definition of the corresponding `pp_expect` function is not
...
A post-processing method relying directly on `pp_expect` is `conditional_effects`,
...
`inst/chunks`.
* Add functions `posterior_predict_foo`, `pp_expect_foo` and `log_lik_foo` to
`posterior_predict.R`, `pp_expect.R` and `log_lik.R`, respectively.
