...
predicted), their theoretical lower and upper bounds (only applied if parameters
are not predicted), information on whether the distribution is discrete or
...
If you want to support `pointwise` evaluation as well, please write
`mu <- brms:::get_dpar(prep, "mu", i = i)` and
