...
predictor, predictor categories (or integers) are not assumend to be equidistant
...
categories. For a single monotonic predictor, $x$, the linear predictor term of
...
The monotonic and the unorderd factor model have almost identical fit in this
...
Dirchlet distribution, a multivariate generalization of the beta distribution.
...
We have used `sample_prior = TRUE` to also obtain samples from the prior
...
Approach for Including Ordinal Predictors in Regression Models](https://psyarxiv.com/9qkhj/). *PsyArXiv preprint*.
