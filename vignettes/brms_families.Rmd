...
where $s_i = g(\eta_i) / \Gamma(1 - 1 / \nu)$ is a positive scale parameter and $\nu > 1$ is a shape parameter so that $g(\eta_i)$ predicts the mean of the Frechet distribution.
...
$$f(y_i) = \frac{p (1 - p)}{\sigma} \exp\left(-\rho_p\left(\frac{y_i - g(\eta_i)}{\sigma}\right)\right)$$
